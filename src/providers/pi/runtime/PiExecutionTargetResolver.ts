...
import type {
  PiExecutionPlatformFamily,
  PiExecutionPlatformOs,
  PiExecutionTarget,
} from './piLaunchTypes';

export interface ResolvePiExecutionTargetOptions {
...
function resolveHostPlatformOs(hostPlatform: NodeJS.Platform): PiExecutionPlatformOs {
...
function resolveHostPlatformFamily(hostPlatform: NodeJS.Platform): PiExecutionPlatformFamily {
...
export function resolvePiExecutionTarget(
  options: ResolvePiExecutionTargetOptions,
): PiExecutionTarget {
  ...
  const piSettings = getPiProviderSettings(options.settings);
...
export async function resolvePiExecutionTargetAsync(
  options: ResolvePiExecutionTargetOptions,
): Promise<PiExecutionTarget> {
  const hostPlatform = options.hostPlatform ?? process.platform;
  if (hostPlatform !== 'win32') {
    return {
      method: 'host-native',
