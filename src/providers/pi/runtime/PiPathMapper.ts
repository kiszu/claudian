...
import type { PiExecutionTarget, PiPathMapper } from './piLaunchTypes';
...
function createIdentityMapper(target: PiExecutionTarget): PiPathMapper {
...
function createWslPathMapper(target: PiExecutionTarget): PiPathMapper {
  const toTargetPath = (hostPath: string): string | null => {
    if (!hostPath) {
      return null;
    }
...
export function createPiPathMapper(target: PiExecutionTarget): PiPathMapper {
